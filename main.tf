...
locals {
  ...
  salt_instance      = yandex_compute_instance.salt.network_interface.0.ip_address
}
...
resource "local_file" "hosts-ini" {
  ...
  content = templatefile("hosts.tftpl", {
    ...
    salt_instance      = local.salt_instance
...
resource "yandex_compute_instance" "salt" {
  platform_id = "standard-v1"
  hostname    = "salt"
...
resource "yandex_vpc_security_group" "custom_sg" {
  ...
  dynamic "ingress" {
    for_each = ["80", "443", "22", "3306", "33060", "4567", "4444", "4568", "6032", "6033", "4505", "4506"]
    content {
      protocol       = "TCP"
